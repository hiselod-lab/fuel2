...
@st.cache_data
def enhanced_feature_engineering(df, forecasting_mode=False, random_state=None):
    """Create deterministic features for model training and forecasting.
    
    Args:
        df: DataFrame with sales data
        forecasting_mode: If True, use realistic random factors for forecasting
    ...
    # Enhanced volume features with momentum and acceleration
    df['volume_change'] = df['sales_volume'].pct_change()
    
    # Calculate momentum features (rate of change of change)
    df['volume_momentum'] = df['volume_change'].pct_change()
    df['price_momentum'] = df['price_change'].pct_change()
    
    # Lag features to capture recent patterns
    for lag in [1, 2, 3, 4]:
        df[f'volume_lag_{lag}'] = df.groupby(['Region', 'Product'])['sales_volume'].shift(lag)
        df[f'price_lag_{lag}'] = df.groupby(['Region', 'Product'])['avg_price'].shift(lag)
    ...
    # Calculate dynamic rolling features with proper handling for forecasting
    for window in [4, 8, 12]:
        ...
        # Rolling trends (slope of linear regression)
        df[f'roll{window}_trend'] = df.groupby(['Region', 'Product'])['sales_volume'].transform(
            lambda x: x.rolling(window, min_periods=2).apply(
                lambda y: np.polyfit(range(len(y)), y, 1)[0] if len(y) >= 2 else 0, raw=False
            )
        )
    ...
    # Dynamic trend features that update properly during forecasting
    ...
    # Recent vs historical comparison features
    df['recent_vs_long_term'] = df['roll4_mean'] / (df['roll12_mean'] + 1e-10)
    df['short_vs_medium_term'] = df['roll4_mean'] / (df['roll8_mean'] + 1e-10)
    
    # Acceleration features (second derivative)
    df['volume_acceleration'] = df['volume_momentum'].pct_change()
    
    # Generate external factors with controlled randomness
    if forecasting_mode:
        # Use more controlled random factors for forecasting
        # These should vary but not dominate the learned patterns
        df['weather_factor'] = np.random.normal(1, 0.05, len(df))
        df['economic_factor'] = np.random.normal(1, 0.02, len(df))
        df['event_factor'] = np.random.normal(1, 0.08, len(df))
    else:
        # Use fixed seed for training to ensure reproducibility
...
def train_models(df, feature_cols, params, train_ratio=0.8, split_method='time'):
    """Train and evaluate models, then retrain on full dataset for forecasting.
    
    This function follows a two-step process:
    1. Split data, train models, and evaluate performance on test set
    2. Retrain models on the full dataset to incorporate all available information
    ...
    Args:
        ...
        split_method: 'time' for chronological split, anything else for random split
    ...
    # Step 2: Retrain models on full dataset for forecasting
    # This ensures all available data is used for future predictions
    ...
    if selector is not None:
        ...
        if isinstance(X_full_sel, np.ndarray):
            if X_full_sel.ndim == 1:
                print(f"Feature selection produced a 1D array with shape {X_full_sel.shape}. Reshaping to 2D.")
                X_full_sel = X_full_sel.reshape(1, -1)
            elif X_full_sel.ndim > 2:
                print(f"Feature selection produced a {X_full_sel.ndim}D array with shape {X_full_sel.shape}. Reshaping to 2D.")
...
def generate_forecast(model_bundle, history, steps, include_confidence=True, confidence_level=0.95):
    ...
    1. Properly recalculates all features at each forecast step using enhanced_feature_engineering
    2. Uses realistic random factors during forecasting (forecasting_mode=True)
    3. Ensures temporal features are updated correctly
    4. Calculates confidence intervals based on model uncertainty
    ...
    for step in range(steps):
        ...
        # This is a key improvement - we regenerate all features at each step
        ...
        if selector is not None:
            ...
            if isinstance(features_sel, np.ndarray):
                if features_sel.ndim == 1:
                    print(f"Feature selection produced a 1D array with shape {features_sel.shape}. Reshaping to 2D.")
                    features_sel = features_sel.reshape(1, -1)
                elif features_sel.ndim > 2:
                    print(f"Feature selection produced a {features_sel.ndim}D array with shape {features_sel.shape}. Reshaping to 2D.")
        ...
        print(f"Generate forecast: Input shape before reshape: {features_sel.shape}, ndim: {features_sel.ndim}")
        # Force reshape to 2D regardless of original shape
        if isinstance(features_sel, np.ndarray):
            # For numpy arrays
            if features_sel.ndim == 1:
                # 1D array to 2D
                ...
                # Higher dimensional array to 2D
                # Preserve first dimension, flatten all others
                features_sel = features_sel.reshape(features_sel.shape[0], -1)
        else:
            # For non-numpy arrays (e.g., sparse matrices)
            features_sel = np.asarray(features_sel)
            if features_sel.ndim == 1:
                features_sel = features_sel.reshape(1, -1)
            elif features_sel.ndim > 2:
                features_sel = features_sel.reshape(features_sel.shape[0], -1)
        print(f"Generate forecast: New shape after reshape: {features_sel.shape}")
...
def generate_detailed_forecast(res, group_eng, product, forecast_weeks, forecast_method,
    ...
    This improved implementation:
    1. Properly recalculates all features at each forecast step
    2. Uses realistic random factors during forecasting
    3. Ensures temporal features are updated correctly
    4. Calculates confidence intervals based on model uncertainty with increasing width over time
    5. Handles both direct and recursive forecasting strategies
    ...
    if forecast_method == 'Direct':
        ...
        for i in range(forecast_weeks):
            ...
            # `future_data` is already a DataFrame with a single row; wrapping it in
            # `pd.DataFrame([future_data])` creates a DataFrame containing another
            # DataFrame object, which leads to a 3D array (shape `(1, 1, n)`)
            # when passed to downstream preprocessing steps. This was the source of
            # the "Must pass 2-d input" errors during forecasting. Concatenating the
            # DataFrames directly preserves the expected 2D structure.
            ...
            # This ensures all temporal features are properly calculated
            ...
            # Calculate historical trend for more realistic forecasts
            if len(y_test) > 1:
            ...
            # Apply product-specific adjustments with increasing effect over forecast horizon
            # These adjustments help model the specific behavior of different products
            if is_hobc:
                # HOBC typically has higher volatility
                volatility_factor = 0.02 * (i + 1) * 3.0
                trend_factor = 0.03 * (i + 1) * 3.0
            else:
                volatility_factor = 0.02 * (i + 1)
                trend_factor = 0.03 * (i + 1)
                
            # Apply trend bias based on historical trend
            trend_bias = historical_trend * trend_strength * 0.01 * (i + 1)
            
            # Fine-tune specific features if they exist in the dataset
            # This preserves domain-specific knowledge about feature relationships
            ...
            if feature_selection_enabled and selector is not None:
                ...
                if isinstance(X_future_sel, np.ndarray):
                    if X_future_sel.ndim == 1:
                        print(f"Feature selection produced a 1D array with shape {X_future_sel.shape}. Reshaping to 2D.")
                        X_future_sel = X_future_sel.reshape(1, -1)
                    elif X_future_sel.ndim > 2:
                        print(f"Feature selection produced a {X_future_sel.ndim}D array with shape {X_future_sel.shape}. Reshaping to 2D.")
            ...
            # Ensure input is 2D for model prediction
            print(f"Direct forecasting: Input shape before reshape: {X_future_sel.shape}, ndim: {X_future_sel.ndim}")
            # Force reshape to 2D regardless of original shape
            if isinstance(X_future_sel, np.ndarray):
                # For numpy arrays
                if X_future_sel.ndim == 1:
                ...
                # For non-numpy arrays (e.g., sparse matrices)
                X_future_sel = np.asarray(X_future_sel)
                if X_future_sel.ndim == 1:
                    X_future_sel = X_future_sel.reshape(1, -1)
                elif X_future_sel.ndim > 2:
                    X_future_sel = X_future_sel.reshape(X_future_sel.shape[0], -1)
            print(f"Direct forecasting: New shape after reshape: {X_future_sel.shape}")
        ...
        # Create a copy of the historical data to build upon
        forecast_history = group_eng.copy()
        prediction_history = []
        ...
        for i in range(forecast_weeks):
            # Create a new row for the forecast date
            if i == 0:
                # For the first forecast, start with the last historical data point
                current_data = last_data.copy()
                current_data['week_start'] = forecast_dates[i]
            else:
                # For subsequent forecasts, use the previous forecast as a starting point
                current_data = forecast_history.iloc[-1:].copy()
                current_data['week_start'] = forecast_dates[i]
            
            # Ensure date is in datetime format
            current_data['week_start'] = pd.to_datetime(current_data['week_start'])
            
            # Add the current forecast point to the history
            # `current_data` is already a one-row DataFrame. Using
            # `pd.DataFrame([current_data])` nests it inside another DataFrame,
            # producing a column of DataFrame objects and ultimately a 3D structure
            # that breaks downstream transformations. Concatenate the DataFrames
            # directly to maintain the expected 2D shape.
            forecast_history = pd.concat([forecast_history, current_data], ignore_index=True)
            
            # Apply enhanced feature engineering with forecasting_mode=True
            # This properly recalculates all time-dependent features
            temp_history = enhanced_feature_engineering(forecast_history, forecasting_mode=True)
            
            # Get the last row which contains our forecast features
            current_data = temp_history.iloc[-1:].copy()
            ...
            # Ensure input is 2D for model prediction
            print(f"Recursive forecasting: Input shape before reshape: {X_future_sel.shape}, ndim: {X_future_sel.ndim}")
            # Force reshape to 2D regardless of original shape
            if isinstance(X_future_sel, np.ndarray):
                # For numpy arrays
                if X_future_sel.ndim == 1:
                ...
                if X_future_sel.ndim == 1:
                    ...
                    X_future_sel = X_future_sel.reshape(X_future_sel.shape[0], -1)
            print(f"Recursive forecasting: New shape after reshape: {X_future_sel.shape}")
            ...
            # Get the base forecast value
            forecast_value = ensemble_pred_future[0]
            
            # Apply trend and seasonality adjustments based on previous predictions
            if i > 0 and len(prediction_history) > 0:
                # Calculate trend direction from prediction history
                trend_direction = 1 if prediction_history[-1] > prediction_history[0] else -1
                
                # Extract seasonal factors if available
                seasonal_sin = current_data['seasonal_sin'].iloc[0] if 'seasonal_sin' in current_data.columns else 0
                monthly_sin = current_data['monthly_sin'].iloc[0] if 'monthly_sin' in current_data.columns else 0
                seasonal_factor = (seasonal_sin + monthly_sin) / 2
                
                # Apply product-specific adjustments
                if product == 'HOBC':
                    # HOBC typically has higher volatility
                    rand_factor = np.random.normal(trend_direction * 0.05 + seasonal_factor * 0.03, 0.08 * (1 + i * 0.1))
                else:
                    rand_factor = np.random.normal(trend_direction * 0.03 + seasonal_factor * 0.02, 0.05 * (1 + i * 0.1))
                
                # Apply the adjustment with a minimum value safeguard
